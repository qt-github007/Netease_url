...
#### 📝 按歌名查找后批量解析
1. 在“单曲解析”中，每行输入一个歌曲名称，或上传每行一个名称的 `.txt` 文件
2. 建议写成“歌名 | 歌手”，例如 `晴天 | 周杰伦`，可提高匹配准确率
3. 点击**查找歌曲 ID**，检查匹配出的歌曲、歌手、专辑、ID 和网易云链接；可通过下拉框改选其他候选版本
4. 取消不准确结果的勾选，然后点击**解析已选歌曲**
5. 解析完成后可直接点击**批量下载成功歌曲**下载 ZIP

一次最多查找 200 个歌曲名称。系统默认取每个名称最相关的一首，未找到的名称会单独显示。
