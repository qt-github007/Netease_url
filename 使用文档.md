...
#### 文件批量解析与下载
1. 进入「单曲解析」功能并选择音质
2. 上传 `.txt`、`.csv` 或 `.xlsx` 歌曲列表文件
3. TXT/CSV 每行填写一个歌曲 ID 或网易云单曲链接；Excel 使用“歌曲 ID”或“网易云链接”列
4. 点击「上传并批量解析」查看逐首解析结果
5. 点击「批量下载成功歌曲」，下载 ZIP 压缩包
...
3. 点击「查找歌曲 ID」，核对匹配歌曲、歌手、专辑、ID 和网易云链接；可通过下拉框改选其他候选版本
4. 取消不准确结果的勾选，点击「解析已选歌曲」
5. 解析完成后点击「批量下载成功歌曲」下载 ZIP
